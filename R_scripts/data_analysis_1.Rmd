...
```{r}
library(knitr)
vals <- c(1,5,10,1,0,0,1,0,100,500,1,0)  
...
# Basic questions
## 1. Display frame number as stored by snow-dots
```{r}
axes_labels_font <- 18
axes_font <- 15
...
```{r}
skipSize <- c(data$onsetFrame[1],diff(data$onsetFrame)) - 1 # subtract 1 as no frame is skipped if skipSize = 1
offset <- 0.5
